...
| `MAX_SPACES` | 1000 | 空间总数上限（防匿名批量建空间） |

### 安全建议（必读）

- **默认无任何认证**：不设 `API_KEY` 且空间无密码时，知道地址的人都能读写，
  服务端会变成公网免费图床。请至少做其中一项：设强 `API_KEY`、
  给空间设密码、或用防火墙限制来源 IP。
- **务必走 wss**：服务端只听 ws，明文流量经过公网可被窃听；按上节 Nginx
  出 `wss://`，客户端填 wss 地址。
- **保护数据目录**：空间密码明文存在 `DATA_DIR/spaces/*.json`，文件明文
  落盘。目录权限收紧（如 `chmod 700`），备份同样按敏感数据处理；
  客户端的密码也明文存在本机偏好设置里，丢机=泄漏。
- 二进制用 debug key 签名，仅适合自用分发；上架应用商店前换正式签名。
- 客户端只信任你自己的服务端：恶意服务端可推送任意剪切板/文件元数据
  （客户端已做帧大小上限、下载越界/停滞保护、TUI 另有 ANSI/路径穿越清洗），
  但不要连来历不明的公共中转服。
...
| 设备列表来回跳动（反复上下线） | 同一设备 ID 在两端同时在线，多因**换机克隆**把应用数据（含设备 ID）一起搬过去了：在其中一台的「设置」最下方点「重置设备标识」即可一键自救 |
| `设备已在别处在线` | 同上，被顶掉的那台会停止自动重连，重置设备标识后手动重连 |
